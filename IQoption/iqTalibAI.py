...
def place_option(Money,goal,expirations_mode):  
    ...
    if close_val>ma_close_val:
        print("pips : %s",(close_val-ma_close_val)*10000)
    ...
    #PUT OPTION
        ...
        print("pips %s",abs(close_val-ma_close_val)*10000)
...
def OnTick():
    while(True):
        dd=Iq.get_realtime_candles(goal,1)
        for k in list(dd.keys()):
            open=(dd[k]['open'])
            close=(dd[k]['open'])
            print("Open : %s | Close: %s"%open,close)
threading.Thread(target=OnTick).start()

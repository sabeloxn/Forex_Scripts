...
namespace cAlgo.Robots
{
    ...
    public class CleanChartTraderv210 : Robot
   {
        ...
        if(BarType=="Red Bar")
        {
            ...
            if(ClosePrice<SellPotentialEntry && BuyLimitNotPlaced && BelowPSAR )
            {
                ...
                var LotsInUnits = Symbol.QuantityToVolumeInUnits(LotSize);
                PlaceLimitOrder(TradeType.Sell,SymbolName,LotsInUnits,SellPotentialEntry-0.3,"SellStop",MaxStopLoss,TakeProfit);
        ...
        else
        {
            ...
            if(ClosePrice>BuyPotentialEntry && SellLimitNotPlaced && AbovePSAR )
            {
                ...
                var LotsInUnits = Symbol.QuantityToVolumeInUnits(LotSize);
                PlaceLimitOrder(TradeType.Buy,SymbolName,LotsInUnits,BuyPotentialEntry+0.3,"BuyStop",MaxStopLoss,TakeProfit);

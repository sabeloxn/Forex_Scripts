...
namespace cAlgo.Robots
{
    ...
    public class Blackgoldx : Robot
    {
        [Parameter("Volume")]
        public double volume { get; set; }
        [Parameter("Quote DP")]
        public int qdpoint { get; set; }
        //------------------------------------------------CHART TF PARAMETERS----------------------------------------------------------------
        [Parameter("Use chart TF", DefaultValue = true)]
        public bool useChart_tf { get; set; }
        ...
        [Parameter("Stop Buffer", DefaultValue = 3.0, MinValue = 1.0)]
        public double stopBuffer { get; set; }

        [Parameter("Volatility ", DefaultValue = 3.0, MinValue = 1.0)]
        public double volatilityThreshold { get; set; }

        [Parameter("longBuffer", DefaultValue = 1.0, MinValue = 1.0)]
        public double longBuffer { get; set; }

        [Parameter("ShortBuffer", DefaultValue = 1.0, MinValue = 1.0)]
...
//-----------------------------------------------STF Parameters-------------------------------------------------------------------------------
        ...
        [Parameter("stfVolatility ", DefaultValue = 3.0, MinValue = 0.01)]
        ...
        [Parameter("Stop Loss", DefaultValue = 1, MinValue = 1)]
        public double stfstopLoss { get; set; }

        [Parameter("stfStop Buffer", DefaultValue = 3.0, MinValue = 1.0)]
        public double stfstopBuffer { get; set; }

        [Parameter("Take Profit", DefaultValue = 1, MinValue = 1)]
        public double stftakeProfit { get; set; }
...
//------------------------------------------------------------------------------------------------------------------------------------        
        ...
        protected override void OnStart()
        {
            ...
            //LotSize = Account.Balance * 100 / 100;
        ...
        protected override void OnBar()
        {
            ...
            LotSize = (long)Math.Ceiling(Account.Balance / volume) * 100;
            stfLotSize = (long)Math.Ceiling(Account.Balance / volume) * 100;
            ...
            double volatility = Math.Round(haHigh - haLow, qdpoint);
            double stfvolatility = Math.Round(stfhaHigh - stfhaLow, qdpoint);
                ...
                if (stfindex != -1)
                {
                    ...
                    if (stflongPreviousClose > stfhaHigh)
                    {
                        ...
                        if (stflongPosition == null && stfhasCrossedAbove == false)
                        {
                            ...
                            Print("stf Volatility : {0}", stfvolatility);
                    ...
                    if (stfshortPreviousClose < stfhaLow)
                    {
                        ...
                        {
                            Print("Volatility : {0}", stfvolatility);
                            if (stfvolatility <= stfvolatilityThreshold)
                            {

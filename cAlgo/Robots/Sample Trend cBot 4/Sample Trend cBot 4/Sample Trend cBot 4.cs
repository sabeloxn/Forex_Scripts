                ...
                if (previousClose < haLow)
                {
                    ...
                    if (longPosition == null && hasCrossedBelow == false)
                    {
                        if (volatility <= volatilityThreshold)
                        {
                            ...
                            var sell = ExecuteMarketOrder(TradeType.Sell, SymbolName, Short_volume, "Sell", ShortStopLoss, null);
                            hasCrossedBelow = true;
                ...
                if (previousClose > haHigh)
                {
                    ...
                    if (longPosition == null && hasCrossedAbove == false)
                    {
                        if (volatility <= volatilityThreshold)
                        {
                            ...
                            var buy = ExecuteMarketOrder(TradeType.Buy, SymbolName, Long_volume, "Buy", LongStopLoss, null);
                            hasCrossedAbove = true;
                            Stopwatch timer = new Stopwatch();
                            int seconds = 30;

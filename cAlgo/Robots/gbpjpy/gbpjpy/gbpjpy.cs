...
namespace cAlgo.Robots
{
    ...
    public class gbpjpy : Robot
    {
        ...
        public string fromAddress = "sabeloxnxele@gmail.com";
        //Sender's Address
        public string toAddress = "sabeloxnxele@gmail.com";
        // Recipient's Address
        public string subject = "Testing email";
        //Email Subject
        public string text = "This email was sent from cAlgo.";
        //Email Body
        protected override void OnStart()
        {
            ...
            Notifications.SendEmail(fromAddress, toAddress, subject, text);
        ...
        protected override void OnBar()
        {
            ...
            double LongStopLoss = Math.Round(Math.Abs(previousClose - haLow) * 100, 2);
            ...
            //LotSize = Convert.ToInt32(Account.Balance * 100) / 100;

            double Risk = Math.Ceiling(Account.Balance * 10) / 100;
            int Lot = Convert.ToInt32(Risk.ToString().Substring(0, 3));
            LotSize = Convert.ToInt32(Lot) * 1000;

            //Convert.ToInt32((Account.Balance * 10) / 100) * 100;
            //------------------------LONG-------------------------------------------------------------
            if (previousClose > haHigh)
            {
                Print("Long Volatility Threshold:{1} ; Long Volatility : {0}", volatility, volatilityThreshold);
            ...
            {
                Print("Short Volatility Threshold:{1} ; Short Volatility : {0}", volatility, volatilityThreshold);

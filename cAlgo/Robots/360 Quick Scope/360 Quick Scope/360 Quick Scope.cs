    ...
    public class 360QuickScope : Robot
    {
        ...
        private void ExecuteOrder(double quantity)
        {
            ...
            if(ExecuteOrders)
            {
                ...
                ExecuteMarketOrder(TradeType.Sell, SymbolName, volumeInUnits, "360BuyMarket", StopLoss, TakeProfit);
                }
                else if(SellSignalRecieved==Bars.HighPrices.Last(1))
                {
                double Stoploss = (SellEntry+SellStoploss)*10000;
                ExecuteMarketOrder(TradeType.Buy, SymbolName, volumeInUnits, "360SellMarket", StopLoss, TakeProfit);
                }   
            }
        }
        ...
        var volumeInUnits = Symbol.QuantityToVolumeInUnits(quantity);
            ...
            if(PlaceOrders)
            {
                ...
                {//Print(SellSignalRecieved +"||"+Bars.HighPrices.Last(1));
                  ...
                  PlaceStopLimitOrder(TradeType.Buy,SymbolName,volumeInUnits,SellEntry,0,"360buy",TakeProfit,StopLoss);
                ...
                else if(BuySignalRecieved==Bars.LowPrices.Last(1))
                {
                  ...
                  PlaceStopLimitOrder(TradeType.Sell,SymbolName,volumeInUnits,BuyEntry,0,"360sell",TakeProfit,StopLoss);

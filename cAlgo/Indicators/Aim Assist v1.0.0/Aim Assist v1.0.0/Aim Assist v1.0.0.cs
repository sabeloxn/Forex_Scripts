...
namespace cAlgo.Indicators
{
    ...
    public class AimAssistv100 : Indicator
    {
       ...
       //Entry Prices and Stop Losses
        ...
        public override void Calculate(int index)
        {
            ...
            //BuySignalSentToBot[index]=BuySignalSent.LastValue;
            //SellSignalSentToBot[index] = SellSignalSent.LastValue;
            //Print("Buy "+BuySignalSent.LastValue+"sell "+SellSignalSent.LastValue);
        ...
        //bool NewBar = index > LastCalculatedIndex;
            ...
            GetAndStoreBars(index);
           // }
            BareBones(UseMa,UseLRF,UseStoch,UseSAR,index);
        }
        
        void GetAndStoreBars(int index)
        {
        ...
        if (bearishBar)
            {
                ...
                Bearhigh = Math.Max(Bearhigh, _Series.HighPrices[index]);
                Bearlow = Math.Min(Bearlow, _Series.LowPrices[index]);
                bearLows[index] = Math.Min(Bearlow, _Series.LowPrices[index]);   
                bearHighs[index] = Math.Max(Bearhigh, _Series.HighPrices[index]);
        ...
        if (bullishBar)
            {
                ...
                Bullhigh = Math.Max(Bullhigh, _Series.HighPrices[index]);
                Bulllow = Math.Max(Bulllow, _Series.LowPrices[index]);
                bullHighs[index] = Math.Max(Bullhigh, _Series.HighPrices[index]);
                bullLows[index] = Math.Max(Bulllow, _Series.LowPrices[index]);
